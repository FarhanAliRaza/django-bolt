...
def parse_msgspec_decode_error(error: Exception, body_bytes: bytes) -> dict[str, Any]:
    ...
    if match:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if current_pos + line_len > byte_pos:
                line_num = i
                col_num = (
                    byte_pos - current_pos + 1
                )  # +1 for human-readable column (1-indexed)
        ...
        return {
            ...
            "msg": msg,
            "input": body_bytes.decode("utf-8", errors="replace")[:200]
            if body_bytes
            else "",
            "ctx": {
                "line": line_num,
    ...
    return {
        ...
        "loc": ("body",),
        "msg": error_msg,
        "input": body_bytes.decode("utf-8", errors="replace")[:100]
        if body_bytes
        else "",
    }
