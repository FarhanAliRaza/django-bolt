...
def compile_middleware_meta(
    ...
    if auth is not None:
        ...
        auth_backends.extend(
            auth_backend.to_metadata()
    ...
    if guards is not None:
        ...
        for guard in guards:
            ...
            if hasattr(guard, "to_metadata") and callable(
                getattr(guard, "to_metadata", None)
            ):
                try:
    ...
    if (
        not all_middleware
        and not auth_backends
        and not guard_list
        and not skip_middleware
    ):
        return None
...
def add_optimization_flags_to_metadata(
    metadata: dict[str, Any] | None, handler_meta: dict[str, Any]
) -> dict[str, Any]:
    """
