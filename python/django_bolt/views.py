...
class ViewSet(APIView):
    ...
    # Note: queryset is defined as a property below, but subclasses can set it as a class attribute
    # The __init_subclass__ hook converts class-level queryset to _base_queryset
    serializer_class: type | None = None
    list_serializer_class: type | None = (
        None  # Optional: override serializer for list operations
    )
            ...
            if instance is not None:
                if many:
                    ...
                    if hasattr(serializer_class, "from_model"):
                        ...
                        return [
                            msgspec.convert(
                                {name: getattr(obj, name, None) for name in fields},
                                serializer_class,
                            )
                            for obj in instance
                        ]
                    ...
                    if hasattr(serializer_class, "from_model"):
                        ...
                        fields = getattr(serializer_class, "__annotations__", {})
                        mapped = {
                            name: getattr(instance, name, None) for name in fields
                        }
...
class ModelViewSet(ViewSet):
    ...
    async def retrieve(self, request, **kwargs):
        ...
        if lookup_value is None:
            raise HTTPException(
                status_code=400, detail=f"Missing lookup field: {self.lookup_field}"
            )
