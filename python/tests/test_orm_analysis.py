...
def _helper_function_with_orm():
    """Helper function that performs ORM operations."""
    from django.contrib.auth.models import User  # noqa: PLC0415

    return User.objects.exists()


def _helper_function_with_sleep():
...
class ServiceLayer:
    """A sample service class to test method traversal."""

    def instance_method_orm(self):
        from django.contrib.auth.models import User  # noqa: PLC0415

        return User.objects.create(username="service_user")

    @staticmethod
    def static_method_orm():
        from django.contrib.auth.models import User  # noqa: PLC0415

        return User.objects.all()

    def deep_nested_entry(self):
        return self._internal_helper()

    def _internal_helper(self):
        # A method calling another internal method
        return self.static_method_orm()


def sync_handler_nested_call():
    """Handler that calls a separate function defined in the module."""
    return _helper_function_with_orm()


def sync_handler_class_instance():
    """Handler that instantiates a class and calls a method."""
    # Pattern: ServiceLayer().instance_method_orm()
    return ServiceLayer().instance_method_orm()


def sync_handler_static_method():
    """Handler that calls a static method on a class."""
    # Pattern: ServiceLayer.static_method_orm()
    return ServiceLayer.static_method_orm()


def sync_handler_deep_recursion():
    """Handler with multiple levels of nesting."""
    # handler -> ServiceLayer() -> deep_nested_entry -> _internal_helper -> static_method_orm -> ORM
    service = ServiceLayer()
    return service.deep_nested_entry()


def sync_handler_nested_blocking():
    """Handler that calls a function which blocks."""
    _helper_function_with_sleep()
    return "done"


class TestComplexHandlerAnalysis:
    """
    Tests for advanced scenarios:
    - Recursive function calls
    - Class method traversal (instantiation and static)
    - Deeply nested blocking operations
    """

    def test_nested_function_call_detection(self):
        """Test that AST visitor follows calls to other functions in the module."""
        analysis = analyze_handler(sync_handler_nested_call)

        assert analysis.uses_orm is True
        assert "exists" in analysis.orm_operations
        assert analysis.is_blocking is True

    def test_class_instance_method_detection(self):
        """
        Test detection of ORM usage via class instantiation.
        Pattern: ServiceLayer().instance_method_orm()
        """
        analysis = analyze_handler(sync_handler_class_instance)

        assert analysis.uses_orm is True
        assert "create" in analysis.orm_operations

    def test_static_class_method_detection(self):
        """
        Test detection of ORM usage via static class method.
        Pattern: ServiceLayer.static_method_orm()
        """
        analysis = analyze_handler(sync_handler_static_method)

        assert analysis.uses_orm is True
        assert "all" in analysis.orm_operations

    def test_nested_blocking_io_detection(self):
        """Test that blocking I/O is detected even inside helper functions."""
        analysis = analyze_handler(sync_handler_nested_blocking)

        assert analysis.has_blocking_io is True
        assert "time.sleep" in analysis.blocking_operations
        assert analysis.is_blocking is True

    # Note: This test verifies the specific capability of tracking 'self' or deeper chains.
    # Based on our implementation, this might require the visitor to track 'self' context.
    # If the current implementation supports deep object recursion, this should pass.
    def test_deep_method_chaining(self):
        """
        Test recursion through multiple internal method calls.
        Handler -> deep_nested_entry -> _internal_helper -> ORM
        """
        analysis = analyze_handler(sync_handler_deep_recursion)
